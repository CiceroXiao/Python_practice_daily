...
ITEMS = [
    ...
    'ANIME VHS TAPE', 'JAR OF PICKLES', 'ONE COWBOY BOOT', 'CLEAN UNDERPANTS',
    '5 DOLLAR GIFT CARD'
]
PLACES = [
    ...
    for suspect in SUSPECTS:
        if random.randint(0, 1) == 0:
            ...
            while True:  # Select a random (wrong) item clue.
                ...
                if clues[interviewee][suspect] != ITEMS[SUSPECTS.index(
                        suspect)]:
                    # Break out of the loop when wrong clue is selected.
                    break
...
while True:  # Main game loop.
    ...
    if response == 'J':  # Player accuses this suspect.
        ...
        if thePersonHere == culprit:
            ...
            print(
                'It was {} who had catnapped ZOPHIE THE CAT.'.format(culprit))
        ...
        if thePersonHere not in zophieClues:
            ...
            if zophieClues[
                    thePersonHere] not in knownSuspectsAndItems and zophieClues[
                        thePersonHere] not in PLACES:
        ...
        if thingBeingAskedAbout in (thePersonHere, theItemHere):
            ...
            # Add non-place clues to the list of known things:
            if clues[thePersonHere][
                    thingBeingAskedAbout] not in knownSuspectsAndItems and \
                    clues[
                        thePersonHere][thingBeingAskedAbout] not in PLACES:
